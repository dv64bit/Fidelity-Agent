...
description: Adopt the design language of a reference (screenshot, design mock, live site, or inspiration) and apply it to the user's EXISTING build/codebase — restyling the UI/UX only, without changing application logic, behaviour, structure, routing, data, or features. Use whenever the user wants to "apply this design language to my app", "restyle/re-theme my build to look like this", "adopt this aesthetic", "make my app look like this reference but don't change functionality", "reskin my codebase", or transfer the look-and-feel of an inspiration onto an existing project. Triggers on the /apply-design-language command. This is a PRESENTATION-LAYER transformation — change tokens, type, color, spacing, shape, elevation, motion, and component styling, but never touch business logic, state, APIs, or IA. Distinct from ss-replicate (which clones one literal screen as new code) — this restyles the whole existing app to share a reference's design language. Prefer this when the codebase already exists and only its visual identity should change.
---
